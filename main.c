...
#pragma omp parallel for collapse(2) default(none) private (i,j) shared(height,width,rgb_image,hist) // schedule (,)
	for(i=0; i<height ; i++){
		for(j=0; j<width; j++){
...
//following loop has data dependencies so no parallelism has been applied
    for(k = 1; k < RANGE; k++)
    {
...
#pragma omp parallel for default (none) private (m) shared(cumhistogram,alpha,size) // schedule (,)
    for(m = 0; m < RANGE; m++)
    {
...
#pragma omp parallel for collapse(2)  default(none) private (x,y) shared(height,width,rgb_image,alpha) // schedule (,)
    for(y = 0; y <height ; y++){
        for(x = 0; x < width; x++){
